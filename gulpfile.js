...
// Copy Slim Minified version of Jquery 3.*.* from node_modules
    var stream = gulp.src( paths.node + 'jquery/dist/jquery.min.js' )
        .pipe( gulp.dest( paths.js ) );
...
// Copy BS4 SCSS files
    gulp.src( paths.node + 'bootstrap/scss/**/*.scss' )
        .pipe( gulp.dest( paths.scss + '/bootstrap' ) );
...
gulp.task( 'clean-vendor-assets', function() {
  return del( [paths.dev + '/js/bootstrap/**', paths.scss + '/bootstrap/**'] );
});
